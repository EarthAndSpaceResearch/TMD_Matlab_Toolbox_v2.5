...
xt=0:0.1:10; yt=xt; [xt,yt]=meshgrid(xt,yt);
...
ik=find(mz==1);
h2(ik)=h(ik);
h2(find(h2==0))=NaN+i*NaN;
hi=interp2(X,Y,h2',xti,yt);hi=conj(hi);
if glob==1, h=h0;mz=mz0;end

figure(10); clf
subplot(2,2,1); pcolor(real(hi)); shading interp; caxis([0 2]);colorbar
grid on; set(gca,'layer','top')
title('Real(hi)')

subplot(2,2,2); pcolor(imag(hi)); shading interp; caxis([0 2]); colorbar
grid on; set(gca,'layer','top')
title('Imag(hi)')

subplot(2,2,3); pcolor(abs(hi)); shading interp; caxis([0 2]); colorbar
grid on; set(gca,'layer','top')
title('Magnitude(hi)')
...
%print test_BLinterp.png -f10 -dpng -r300
