...
%            if "OB" mode TS(i,j)- tide at SDtime(i) at location lat(j),lon(j)
...
TimeSeries=0;DriftTrack=0;TMap=0;OB=0;
[N,M]=size(lat);
n1=N*M;
...
% make sure all dimensions correspond
 ...
 elseif n3==1,
  ...
  fprintf('MODE: Map\n');
 elseif n3>1 & n1>1 & n3~=n1,
  OB=1;M=n3;N=n1;
  SDtime=reshape(SDtime,M,1);
  lat=reshape(lat,N,1);
  lon=reshape(lon,N,1);
  fprintf('MODE: OB time series\n');
 else
...
if nargin<=5,
 ...
 elseif OB==1,
  ...
  for k=1:M
   for l=1:N
    TS(k,l)=harp1(time(k),hc(:,l),conList);
    dh = InferMinor(hc(:,l),conList,SDtime(k));
    TS(k,l)=TS(k,l)+dh;
   end
  end
 else % Map
